    ...
    db.getConnection((err, connection) => {
      ...
      connection.beginTransaction((err) => {
        ...
        connection.query(insertQuery, values, (err, insertResult) => {
          ...
          `;
          connection.query(updateEmpQuery, [assignmentData.employee_id, assignmentData.requestid], (err, updateEmpResult) => {
            if (err) {
              return connection.rollback(() => {
                connection.release();
                return reject(err);
            ...
            const updateRequestQuery = `
              ...
              WHERE requestid = ?;
            `;
            connection.query(updateRequestQuery, [assignmentData.requestid], (err, updateRequestResult) => {
              if (err) {
                return connection.rollback(() => {
                  connection.release();
                  return reject(err);
